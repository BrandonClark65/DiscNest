...
export default function DiscBagDisplay({ bag }: DiscBagDisplayProps) {
  ...
  // If bag is passed as prop, use it; else fetch from API
  const [bagDiscs, setBagDiscs] = useState<Disc[]>(bag || []);
  const [visibleCount, setVisibleCount] = useState(0);
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);

  // Only fetch bag if not controlled via prop
  useEffect(() => {
  ...
  return (
    <div className="flex justify-center items-center w-full">
      ...
      >
        ...
        {/* Discs (interactive and in front) */}
        {/* Discs (interactive and in front) */}
        ...
        {isLoggedIn &&
          ...
          {/* Tooltips (top layer, outside disc z-10) */}
          {isLoggedIn &&
            bagDiscs.slice(0, MAX_DISCS).map((disc, i) => {
              if (i >= visibleCount) return null;
              ...
              return (
                  ...
                  key={`tooltip-${disc._id}`}
                    ...
                    style={{
                    top: discTop - 25, // adjust above disc
        ...
        {/* Bag image visually on top, but allows pointer events to pass through */}
        <img
          src="/images/bag-no-discs.png"
          alt="Disc Golf Bag"
          className="absolute top-0 left-0 z-20 w-full h-full pointer-events-none"
