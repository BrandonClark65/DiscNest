...
export default function DiscBagDisplay({ bag }: DiscBagDisplayProps) {
  ...
  useEffect(() => {
    ...
    const fetchIfReady = async () => {
      // prefer useSession session, but if it's undefined for some reason, call getSession()
  ...
  // Layout constants (kept identical to your original)
  const DISC_SIZE = 150;
  ...
  return (
    <div className="flex justify-center items-center w-full">
      <div className="relative transition-opacity duration-700" style={{ width: CONTAINER_WIDTH, height: CONTAINER_HEIGHT }}>
        {/* Background square */}
        <img
        ...
        {/* Login message: show only after useSession() has finished hydrating (session can be undefined initially) */}
        ...
        <div className="relative" style={{ width: CONTAINER_WIDTH, height: CONTAINER_HEIGHT }}>
          {isLoggedIn && (
            // show discs when we actually have any discs (don't rely on bagLoaded)
          ...
          {/* Bag overlay image (still sets bagLoaded but we no longer rely on it for disc visibility) */}
          <img
